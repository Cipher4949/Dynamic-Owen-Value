    ...
    if data_name == "adult":
        ...
        new_data_size_dict = (
            ...
            if data_size_dict["train"] <= 32561 - data_size_dict["test"]
                ...
                "train": 32561 - data_size_dict["test"],
                "test": data_size_dict["test"],
            }
        )
        ...
        if augment_flag:
            ...
            new_train_data.to_csv(
                "./data_files/" + "train_data_augmented.csv", index=False
            )
...
def variance(list1, list2):
    ...
    try:
        if round(float(np.sum(list1)), 10) != round(float(np.sum(list2)), 10):
            raise Exception(
                "[*] Variance is invalid with different means!\n"
...
def generate_union_description(idx_num, union_num):
    idxs = np.arange(idx_num)
    np.random.shuffle(idxs)
    lengths = np.random.randint(idx_num, size=union_num)
    lengths = lengths / np.sum(lengths) * idx_num
    lengths = np.round(lengths).astype(int)
    return np.split(idxs, np.cumsum(lengths)[:-1])
